...
class SpanEvaluator:
    ...
    def compute(self, start_probs, end_probs, gold_start_ids, gold_end_ids):
        ...
        """
        pred_start_ids = get_bool_ids_greater_than(start_probs,limit=0.5)
        pred_end_ids = get_bool_ids_greater_than(end_probs,limit=0.5)
    ...
    def accumulate(self):
        ...
        """
        print(self.num_correct_spans,self.num_infer_spans,self.num_label_spans)
...
def convert_example(example, tokenizer, max_seq_len):
    ...
    for item in example["result_list"]:
        if item['start']>=0:
            start_position=example['content'].find(item['text'])
            end_position=start_position+len(item['text'])
            start = map_offset(start_position + bias, offset_mapping)
            end = map_offset(end_position - 1 + bias, offset_mapping)
            start_ids[start] = 1.0
            end_ids[end] = 1.0
        else:
            start = map_offset( bias, offset_mapping)
            end = map_offset( bias, offset_mapping)
            start=start+item["start"]-1
            end=end+item["end"] -2
            if 'negative'in item['text']:
                start-=1
            start_ids[start] = 1.0
            end_ids[end] = 1.0
    ...
    s_e_ids = [
        np.pad(x, (0, max_seq_len-x.shape[-1]), 'constant') for x in tokenized_output[3:5]]
    tokenized_output = [
        np.pad(x, (0, max_seq_len-x.shape[-1]), 'constant',constant_values=(0,1)) for x in tokenized_output[0:3]]
    tokenized_output.append(s_e_ids[0])
    tokenized_output.append(s_e_ids[1])
...
class Logger(object):
    ...
    def __init__(self, name: str = None,log_path:str=None):
        ...
        if log_path is not None:
            self.file_hander = logging.FileHandler(log_path)
        else:
            self.file_hander = logging.FileHandler('../temp.txt')

        self.file_hander.setFormatter(self.format)
...
def seq_id(seq_ids):
    seq_ids=seq_ids.clone().detach()
    seq_gd_sample=seq_Gd()
    seq_ids=seq_Gd_trans(seq_ids,seq_gd_sample)
    seq_ids=F.softmax(seq_ids)
    return seq_ids
...
def seq_Gd_trans(output_prob,distribute_sample):
    b,l=output_prob.shape
    output_prob_copy=-10000*torch.ones(b,l)
